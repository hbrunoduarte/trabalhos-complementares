...
void atualizaAnimacao(int value) {
    if(animacaoTchau) {
        ...
        angCoxaEsqX = sin(tempoAnim + PI) * 30.0f;

        angJoelhoDirX = 15.0f + sin(tempoAnim) * 15.0f;
        angJoelhoEsqX = 15.0f + sin(tempoAnim + PI) * 15.0F;
    ...
    // Pede pro OpenGL redesenhar a cena (afinal, os ângulos acabaram de mudar!)
    glutPostRedisplay();

    // chama essa mesma função de novo daqui 16 milissegundos
    glutTimerFunc(16, atualizaAnimacao, 1);
}
...
void RenderScene(void) {
    ...
    glPushMatrix();
        ...
        glPushMatrix();
            ...
            glRotatef(angOmbroDirZ, 0.0f, 0.0f, 1.0f);
